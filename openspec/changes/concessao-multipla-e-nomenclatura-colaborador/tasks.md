...
- [ ] 1.1 Em `src/permissions.ts`, trocar `CreateGrantRequest.subjectUserId:
...
- [ ] 1.2 Adicionar o tipo do corpo de recusa por teto,
...
- [ ] 2.1 Acrescentar `grants: { maxSubjects }` a `apps/api/src/config.ts`
...
- [ ] 2.2 Validação de **forma** do corpo (`400`): `subjectUserIds` presente,
  ...
  não-string.
- [ ] 2.3 **Dedup** dos `subjectUserIds` preservando a ordem de chegada, antes de
...
- [ ] 2.4 **Teto** sobre o conjunto deduplicado: `413` com
...
- [ ] 2.5 **Existência dos sujeitos em consulta única** dentro da transação
...
- [ ] 2.6 Substituir o laço de upsert por verbo pelo `INSERT ... SELECT` com
...
- [ ] 2.7 Confirmar que a `unit_id` gravada continua vindo do **recurso** e que
...
- [ ] 3.1 Trocar a emissão única por um laço sobre os destinatários **após o
  ...
  `201` na resposta.
- [ ] 3.2 Verificar a idempotência por destinatário: repetir a mesma concessão
...
- [ ] 3.3 Confirmar que concessão **sem** prazo continua não emitindo aviso algum,
  com qualquer número de destinatários.

## 4. Diálogo de permissões (`apps/web/src/permissoes/`)

- [ ] 4.1 `queries.ts`: ajustar o tipo do corpo de `useCreateGrant` ao novo
  `CreateGrantRequest`. Verificar que `npm run build --workspace apps/web` compila.
- [ ] 4.2 `PermissoesModal.tsx`: trocar o `Select` de pessoa por `mode="multiple"`
...
- [ ] 4.3 Enviar **uma única** `POST /grants` com todos os colaboradores e verbos.
...
- [ ] 4.4 Prévia de prazo vigente por colaborador selecionado que já tenha
...
- [ ] 4.5 Aviso próprio para `413` com `details.error ===
...
- [ ] 4.6 Confirmar que a lista de concessões vigentes e a revogação por verbo
...
- [ ] 5.1 `shell/AppShell.tsx`: item de menu "Pessoas" → "Colaboradores" (a rota
...
- [ ] 5.2 `pessoas/PessoasPage.tsx`: título da página, botão "Nova pessoa" →
...
- [ ] 5.3 `pessoas/PessoaFormModal.tsx`: título "Editar/Nova pessoa" → "Editar/Novo
  colaborador". **Não** alterar `ROLE_LABEL` — o papel continua "Colaborador"
  (design.md D7).
- [ ] 5.4 `pessoas/SenhaGeradaModal.tsx`: "repasse à pessoa" → "repasse ao
  colaborador".
- [ ] 5.5 `auditoria/AuditoriaModal.tsx`: cabeçalho de coluna "Pessoa" →
  "Colaborador".
- [ ] 5.6 `painel/PainelPage.tsx`: "Total de pessoas" → "Total de colaboradores".
- [ ] 5.7 `unidades/UnidadesPage.tsx`: avisos de "pessoas vinculadas" →
  "colaboradores vinculados", nos dois pontos (mensagem de erro e descrição da
  confirmação).
- [ ] 5.8 `permissoes/PermissoesModal.tsx`: rótulo/placeholder do seletor,
...
- [ ] 5.9 Atualizar as âncoras de texto nos testes afetados (`pessoas.test.tsx`,
...
- [ ] 5.10 Passo de conferência (não é gate de CI, design.md D9): rodar
...
- [ ] 6.1 `mkdocs.yml`: rótulo de navegação "Pessoas" → "Colaboradores",
  **mantendo** o caminho `administrador/pessoas.md` (design.md D8, preservação de
  endereço publicado).
- [ ] 6.2 `docs/administrador/pessoas.md`: título e prosa para "Colaborador(es)",
  incluindo o texto do papel, que continua "Colaborador".
- [ ] 6.3 `docs/administrador/permissoes.md`: prosa para "colaborador(es)" **e**
...
- [ ] 6.4 Demais páginas com ocorrências: `index.md`, `a-tela.md`,
...
- [ ] 6.5 Verificar: `grep -rniE "pessoa|servidor" docs/manual/docs` sem ocorrência
...
- [ ] 7.1 `npm run lint && npm run build && npm run test` na raiz, tudo verde.
- [ ] 7.2 `npm run format` e confirmar que `format:check` passa, **sem** formatar
  `openspec/`, `docs/` ou `.claude/` (fora do escopo do Prettier por decisão).
- [ ] 7.3 Subir a app em dev (`npm run dev:api` + `npm run dev:web`), autenticar
...
- [ ] 7.4 No mesmo ambiente, conceder com prazo a três colaboradores e confirmar
  que **cada um** recebe o aviso na central de notificações (entrar com cada
  conta, ou consultar `notifications` no Postgres local).
- [ ] 7.5 `openspec validate --changes concessao-multipla-e-nomenclatura-colaborador
