# visualizacao Specification

## Purpose

Define os requisitos verificáveis da resposta de visualização de arquivo
quando o formato não pode ser renderizado nativamente pelo navegador, no
Épico 9 / **US 9.2, cenário 2** do PRD (`docs/prd_final.md`), RF #16. Esta
capability trata do comportamento da rota `POST /files/:id/view-url` **após**
a autorização do verbo `view` já resolvida pela capability `controle-acesso`
(dono OU grant `view` OU admin da unidade, fail-closed): classifica o formato
do arquivo pelo `content_type` e, quando ele não é pré-visualizável, informa a
indisponibilidade e sinaliza a oferta de download conforme a permissão do
solicitante, em vez de emitir uma URL `inline`. O cenário 1 da US 9.2
(renderização dos formatos suportados e conversão de documentos Office para
PDF via `PreviewConversionPort`) fica fora desta fatia, para fase futura do
MVP.
