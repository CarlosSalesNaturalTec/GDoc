...
- [ ] 1.1 `apps/web/src/lib/schemas.ts`: adicionar `searchFilesResponseSchema`
...
- [ ] 1.2 `apps/web/src/lib/schemas.ts`: adicionar um schema mínimo para a
  ...
  D6
- [ ] 1.3 `apps/web/src/busca/queries.ts`: hook `useSearchFiles(params)` como
...
- [ ] 1.4 `apps/web/src/busca/queries.ts`: hook `useAuthorOptions()` (para o
...
- [ ] 3.2 Botão **"Limpar filtros"** reseta o estado de filtros para vazio; a
  busca sem critérios refaz-se por mudança de `queryKey` (sem caminho separado de
  reset) — spec: limpar filtros; design.md D3
- [ ] 3.3 Tabela de resultados **só-arquivos**: colunas Tipo · Nome · Tamanho ·
...
- [ ] 3.4 Ações da linha reusando `web-visualizacao`: nome clicável e
...
- [ ] 4.1 `apps/web/src/app/router.tsx`: nova rota `/busca` sob `RequireAuth`
  (qualquer papel), dentro do `AppShell` — design.md D1
- [ ] 4.2 `apps/web/src/shell/AppShell.tsx`: item de menu **"Buscar"** (ícone
...
- [ ] 5.1 `apps/web/src/__tests__/busca.test.tsx`: reusar `renderApp` +
...
- [ ] 5.2 Limpar filtros reseta os controles e refaz a busca sem critérios
  (estado inicial permitido) — spec: limpar filtros
- [ ] 5.3 Filtro de autor **aparece para admin** (popula de `GET /users` e envia
  `author`) e **não aparece para colaborador** (sem chamada a `GET /users`) —
  spec: filtro de autor restrito a administrador
- [ ] 5.4 Resultado vazio exibe `Empty`; nome/"Visualizar" abre o `PreviewModal`
...
- [ ] 6.2 `docs/frontend_roadmap.md`: marcar a **Fatia 5** como entregue (✅) e
