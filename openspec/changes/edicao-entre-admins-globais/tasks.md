...
- [ ] 1.1 Desdobrar `canActOnTarget` (`apps/api/src/routes/users.ts`) em
      ...
      transação, como hoje
- [ ] 1.2 Ligar `POST /users/:id/password` a `canResetPasswordOf` com a regra
...
- [ ] 2.2 Recusar a alteração do **próprio status** para desativado, com a mesma
      recusa total, e verificar com teste
- [ ] 2.3 Garantir que reenviar o **mesmo** papel que já se tem não é tratado como
...
- [ ] 3.1 Criar `apps/api/src/__tests__/alcance-admin-global.test.ts` cobrindo os
...
- [ ] 3.2 Cobrir o caso que originou o change: `global_admin` concede cota
      individual a outro `global_admin`, e a si mesmo, verificando que o valor é
      gravado
- [ ] 3.3 Verificar que o invariante "resta ao menos um `global_admin` ativo" se
...
- [ ] 4.1 Acrescentar em `docs/manual/docs/administrador/pessoas.md` a tabela de
...
- [ ] 4.2 Registrar na mesma página que ninguém altera o próprio papel nem
...
- [ ] 5.1 Rodar `npm run lint`, `npm run format:check` e `npm run build` na raiz e
      confirmar saída limpa
- [ ] 5.2 Rodar `npm run test` na raiz e confirmar as duas suítes verdes, com
      atenção aos testes de segurança (`permission`, `isolamento-unidade`,
      `rls-isolation`), que não devem mudar
- [ ] 5.3 Exercitar num banco de dev: um `global_admin` concede cota a outro
      `global_admin` pela API, e a tentativa de redefinir a senha dele continua
      `403`
- [ ] 5.4 Rodar `openspec validate edicao-entre-admins-globais --strict`
