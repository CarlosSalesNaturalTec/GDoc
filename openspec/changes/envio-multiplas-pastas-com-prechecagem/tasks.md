...
- [ ] 1.1 Definir o DTO da resposta de cota (`quotaBytes`, `usedBytes`,
...
- [ ] 1.2 Definir `UPLOAD_SLICE_MAX_ITEMS_DEFAULT = 200` e
...
- [ ] 1.3 Recompilar (`npm run build --workspace packages/shared`).

## 2. Consulta de cota (`apps/api/src/routes/files.ts`)

- [ ] 2.1 Implementar `GET /files/quota` sob `attachTenantContext`, derivando a
...
- [ ] 2.2 Calcular `trashedBytes` a partir dos arquivos do solicitante na
...
- [ ] 2.3 Calcular `pendingBytes` com a mesma soma de `pending`/`replacing` já
  usada na reserva do lote, e descontá-lo de `availableBytes`.
- [ ] 2.4 Verificar com teste que a rota **não** usa o bypass de `global_admin`:
  um admin global recebe a própria cota, não a de outra pessoa nem agregado de
  unidade.
- [ ] 2.5 Verificar com teste de `web-serving.test.ts` que `GET /files/quota`
...
- [ ] 3.1 Montar a área de soltar com travessia de diretório ativa, aceitando
...
- [ ] 3.2 Verificar com teste que `deriveRelativePath` permanece **inalterado** e
  produz o caminho correto para arquivos vindos do `drop`, que chegam com
  `webkitRelativePath` preenchido a partir do `fullPath`.
- [ ] 3.3 Manter os dois botões de envio funcionando em paralelo à área de
...
- [ ] 4.1 Implementar a apuração local de quantidade e soma de tamanhos por
  `File.size`, sem leitura de conteúdo (design.md D3).
- [ ] 4.2 Exibir o estado "analisando seleção" com cancelamento durante a
  travessia — verificar com teste que nenhuma requisição é emitida nessa fase.
- [ ] 4.3 Consultar `GET /files/quota` **após** a apuração e antes do veredito,
  para que o retrato seja o mais recente possível.
- [ ] 4.4 Verificar com teste que uma seleção inviável não emite pedido de URLs e
  não transfere byte algum.

## 5. Veredito ao usuário

- [ ] 5.1 Implementar o painel de recusa com volume pedido, disponível, faltante
  e a decomposição entre ativos, retido na lixeira e pendentes (design.md D4).
- [ ] 5.2 Incluir na recusa a declaração explícita de que excluir arquivos **não**
  libera espaço de imediato, com o prazo de retorno do espaço da lixeira —
  verificar com teste a presença da mensagem.
- [ ] 5.3 Implementar "enviar só o que cabe", identificando com clareza o que
  ficou de fora (ver Open Question do design sobre o critério do subconjunto).
- [ ] 5.4 Implementar a confirmação prévia quando a seleção cabe, com quantidade
  e volume — verificar com teste que nada é transferido antes da confirmação.

## 6. Envio fatiado

- [ ] 6.1 Implementar a divisão em fatias por `UPLOAD_SLICE_MAX_ITEMS` **e**
...
- [ ] 6.2 Pedir as URLs de cada fatia imediatamente antes de transferi-la, nunca
  todas no início — verificar com teste a ordem das requisições.
- [ ] 6.3 Fazer a fila de concorrência (de `corrige-defeitos-envio-lote`)
  atravessar as fatias sem reiniciar — verificar com teste que a transição entre
  fatias não deixa a fila ociosa.
- [ ] 6.4 Remover da SPA a recusa por quantidade introduzida em
...
- [ ] 7.1 Substituir a lista por item por um progresso único do conjunto, medido
  em bytes transferidos sobre bytes totais (design.md D6) — `put-object.ts`
  permanece inalterado; muda o consumidor do `onProgress`.
- [ ] 7.2 Exibir como informação complementar a contagem de concluídos, o arquivo
  corrente e o contador de falhas.
- [ ] 7.3 Implementar a listagem das falhas sob demanda, preservando a nova
  tentativa por item.
- [ ] 7.4 Verificar com teste que o progresso avança proporcional aos bytes num
  conjunto de arquivos de tamanhos muito diferentes, e não em saltos por arquivo.

## 8. Estouro de cota durante o envio

- [ ] 8.1 Pausar o envio quando uma fatia for recusada por cota, sem prosseguir
  para as fatias seguintes (design.md D7).
- [ ] 8.2 Reconsultar `GET /files/quota` e reapresentar o painel detalhado, com a
  quantidade de arquivos que ficou por enviar.
- [ ] 8.3 Sinalizar o envio como incompleto — verificar com teste que os arquivos
  já transferidos permanecem enviados.

## 9. Documentação

- [ ] 9.1 Reescrever `docs/manual/docs/colaborador/enviar.md`: remover a
...
- [ ] 9.2 Ajustar `docs/manual/docs/referencia/limites.md`: o teto de arquivos por
...
- [ ] 9.3 Acrescentar a `docs/manual/docs/referencia/limites.md`, na seção de
  recursos indisponíveis em celular e tablet, a área de arrastar e soltar.
- [ ] 9.4 Acrescentar ao `docs/prd_final.md` a US de envio de várias pastas com
  verificação prévia de viabilidade, com os cenários Dado/Quando/Então
  correspondentes — as specs desta fatia a referenciam.

## 10. Fechamento

- [ ] 10.1 Rodar `npm run lint`, `npm run build`, `npm run test` e
  `npm run format:check` na raiz, com tudo verde.
- [ ] 10.2 Verificar que os testes de isolamento e segurança
