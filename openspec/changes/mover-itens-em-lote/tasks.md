...
- [ ] 1.1 Definir os DTOs de pedido do lote (`ids: string[]`,
...
- [ ] 1.2 Acrescentar o código de erro `move_batch_limit_exceeded`, no molde de
...
- [ ] 1.3 Recompilar `packages/shared` para que api e web enxerguem os tipos
  novos (`npm run build --workspace packages/shared`).

## 2. Configuração (`apps/api`)

- [ ] 2.1 Acrescentar `config.moveBatch.maxItems` lido de `MOVE_BATCH_MAX_ITEMS`
  ...
  sobrescreve.
- [ ] 2.2 Documentar `MOVE_BATCH_MAX_ITEMS` em `.env.example` — verificar que o
  arquivo lista a variável com o valor padrão.

## 3. Rota de lote de arquivos (`POST /files/move`)

- [ ] 3.1 Implementar a rota em `routes/files.ts` com a pré-condição global
...
- [ ] 3.2 Implementar o laço por item dentro de uma única
...
- [ ] 3.3 Gravar auditoria `AuditAction.MOVE` **somente** para os itens com
...
- [ ] 3.4 Verificar por teste que a rota preserva `object_path`, `owner_id`,
...
- [ ] 4.1 Implementar a rota em `routes/folders.ts` com a mesma pré-condição
...
- [ ] 4.2 Implementar o laço por item com `canReorganize`, `wouldCreateCycle`
...
- [ ] 4.3 Chamar `assertNoCycleAfterMove` **uma vez, ao fim**, sobre as pastas
...
- [ ] 4.4 Verificar por teste que mover pasta em lote não grava nenhum evento de
  auditoria, nem para as pastas nem para os arquivos das subárvores
  (design.md D6).

## 5. Roteamento e invariantes de servidor

- [ ] 5.1 Acrescentar teste fixando que `POST /files/move` e `POST /folders/move`
...
- [ ] 5.2 Verificar que `web-serving.test.ts` continua passando sem alteração e
...
- [ ] 5.3 Verificar por teste que `rls-isolation.test.ts`,
...
- [ ] 6.1 Acrescentar `rowSelection` à `Table` do `ExplorerPage` sobre o
...
- [ ] 6.2 Esvaziar a seleção sempre que a pasta corrente mudar — entrar em
...
- [ ] 6.3 Acrescentar a barra de ação em lote, visível a partir de um item
  ...
  esvaziada.
- [ ] 6.4 Verificar por teste que a seleção permanece alcançável abaixo do
...
- [ ] 7.1 Generalizar `MoverItemModal` de `item: MovingItem` para
...
- [ ] 7.2 Acrescentar as mutações de lote em `navegacao/queries.ts` e disparar
...
- [ ] 7.3 Relatar falha parcial informando quantos foram movidos e listando
...
- [ ] 7.4 Recusar o envio com aviso próprio quando a seleção exceder o teto,
  distinguindo-o da recusa por permissão — verificar por teste que nada é enviado
  nesse caso.
- [ ] 7.5 Recarregar a listagem e esvaziar a seleção ao fim de toda operação de
  lote, inclusive parcial — verificar por teste que a seleção fica vazia e a
  listagem é invalidada.
- [ ] 7.6 Verificar por teste que escolher como destino uma pasta selecionada não
  é bloqueado na interface e que o aviso de ciclo vem da recusa do servidor
  (design.md D7).

## 8. Documentação

- [ ] 8.1 Acrescentar a US 2.4 (mover em lote) a `docs/prd_final.md`, com os
...
- [ ] 8.2 Atualizar `docs/manual/docs/colaborador/renomear-e-excluir.md` para
...
- [ ] 8.3 Atualizar `docs/manual/docs/referencia/tarefas-rapidas.md` onde
  descreve mover, incluindo a forma em lote — verificar que o texto continua
  coerente com a página do colaborador.

## 9. Validação final

- [ ] 9.1 Rodar `npm run lint`, `npm run build`, `npm run test` e
  ...
  por decisão.
- [ ] 9.2 Rodar `openspec validate --strict` para a change — verificar que os
  três deltas de spec são aceitos.
- [ ] 9.3 Verificar por inspeção que nenhuma migração SQL, arquivo Terraform ou
