...
- [x] 1.1 Adicionar DTO de resposta da auditoria em `packages/shared/src`
  (item com `actor: { id, name, email }`, `action: 'view' | 'download'`,
  `createdAt: string`) e exportar em `index.ts`
- [x] 1.2 Rebuild do pacote: `npm run build --workspace packages/shared`
  (consumido compilado por `apps/api`)

## 2. Autorização de leitura (lib/access.ts)

- [x] 2.1 Adicionar helper `canReadAudit(client, ctx, fileId)` em
...
- [x] 2.2 Documentar no comentário do helper por que é mais estrito que
...
- [x] 3.1 Criar `apps/api/src/routes/audit.ts` com `GET /files/:id/audit`,
  abrindo sua própria `withTenantTransaction` (padrão das demais rotas tenant)
- [x] 3.2 Autorizar via `canReadAudit`; se negado, responder **403 sem corpo de
  auditoria** (design D4), sem distinguir inexistente/outra-unidade/não-dono
- [x] 3.3 Consultar eventos: `SELECT ae.action, ae.created_at, u.id, u.name,
...
- [x] 3.4 Definir o teto superior como constante interna (design D6, ex.: 500)
- [x] 3.5 Registrar a rota em `apps/api/src/app.ts` sob
  `attachTenantContext(ports)`, junto das demais rotas tenant

## 4. Índice de leitura (avaliar — design D7)

- [x] 4.1 Rodar `EXPLAIN` da consulta da task 3.3 contra dados de teste; decidir
  se `audit_events (file_id, created_at DESC)` compensa nesta fatia
- [x] 4.2 Se sim, criar **nova** migração aditiva
...
- [x] 5.1 Novo arquivo de teste no padrão `seedTwoUnits` / `withSystemBypass`
  (execução sequencial, `fileParallelism: false`)
- [x] 5.2 Dono consulta a auditoria do próprio arquivo e vê os eventos
  `view`/`download` com ator + ação + data/hora, ordenados desc (US 7.2)
- [x] 5.3 Admin da unidade consulta a auditoria de arquivo da unidade do qual
  não é dono e vê os eventos (US 7.1)
- [x] 5.4 Colaborador com grant `view` (não dono, não admin) recebe **403**
  (design D2 — grant não concede auditoria)
- [x] 5.5 Isolamento entre unidades: pessoa da unidade A recebe 403 ao consultar
  auditoria de arquivo da unidade B, sem vazar existência
- [x] 5.6 Arquivo inexistente ⇒ 403; arquivo na lixeira (`deleted_at` setado)
  ⇒ 403 (mesmo resultado, fail-closed)
- [x] 5.7 Arquivo sem eventos ⇒ 200 com lista vazia; e que apenas
  `view`/`download` são retornados (evento não-acesso, se semeado, não aparece)

## 6. Fechamento

- [x] 6.1 `npm run lint && npm run build && npm run test --workspace apps/api`
  verdes
- [x] 6.2 `openspec verify --change epico-7-auditoria-consulta` (ou
