...
- [x] 1.1 Adicionar em `packages/shared/src/dashboard.ts` (ou módulo de tipos
...
- [x] 1.2 Em `packages/shared/src/storage.ts`, definir a união discriminada
  ...
  pelo `index.ts`
- [x] 1.3 `npm run build --workspace packages/shared` para atualizar o `dist`
  consumido por `apps/api`

## 2. Rota de visualização (`apps/api`)

- [x] 2.1 Em `apps/api/src/routes/files.ts`, no handler
...
- [x] 2.2 Ramo pré-visualizável: preservar o comportamento atual —
  `recordAudit(..., VIEW)` + `getViewUrl` — e responder com
  `previewAvailable: true`
- [x] 2.3 Ramo não pré-visualizável: **não** auditar e **não** assinar URL;
...
- [x] 3.1 Formato pré-visualizável (ex.: `application/pdf`, `image/png`): resposta
  `previewAvailable: true` com URL e ação `view`, e **uma** linha de auditoria
  `view` gravada (padrão `seedTwoUnits` / `withSystemBypass`)
- [x] 3.2 Formato Office (ex.: `.docx`) com solicitante detentor de `download`:
  `previewAvailable: false`, sem URL, **nenhuma** auditoria `view`, e
  `download.available === true`
- [x] 3.3 Formato não pré-visualizável com solicitante que só tem `view` (sem
  `download`): `previewAvailable: false` e `download.available === false`
- [x] 3.4 `content_type` ausente/desconhecido: cai no ramo `previewAvailable:
  false` (nunca URL `inline`)
- [x] 3.5 Sem o verbo `view` (arquivo de outra unidade, item na lixeira, link
