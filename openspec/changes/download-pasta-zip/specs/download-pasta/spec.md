...
Capability nova. Implementa o Épico 3 / **US 3.3** do PRD (`docs/prd_final.md`),
adiada no Épico 3 por dependência do motor de permissão e explicitamente deixada
fora do `epico-4-permissoes-granulares`. Os cenários Given/When/Then da US são
vinculantes; os requisitos abaixo os tornam verificáveis. A regra de resolução de
acesso continua sendo a da capability `controle-acesso` — esta capability a
consome item a item, sem afrouxá-la. Ver design.md D1–D8.
