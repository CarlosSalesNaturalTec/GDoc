...
- [ ] 1.1 Definir `UPLOAD_BATCH_MAX_ITEMS_DEFAULT = 500`, no molde de
...
- [ ] 1.2 Acrescentar o código de erro `upload_batch_limit_exceeded` ao DTO de
...
- [ ] 1.3 Recompilar (`npm run build --workspace packages/shared`) para que api e
  web enxerguem os símbolos novos.

## 2. Configuração (`apps/api`)

- [ ] 2.1 Acrescentar `config.requestBodyMaxBytes` lido de
...
- [ ] 2.2 Acrescentar `config.uploadBatch.maxItems` lido de
  `UPLOAD_BATCH_MAX_ITEMS` com padrão `UPLOAD_BATCH_MAX_ITEMS_DEFAULT`, no molde
  de `config.moveBatch` (design.md D2).
- [ ] 2.3 Acrescentar `config.signedUrlUploadTtlSeconds` lido de
...
- [ ] 2.4 Documentar `REQUEST_BODY_MAX_BYTES`, `UPLOAD_BATCH_MAX_ITEMS` e
  `SIGNED_URL_UPLOAD_TTL_SECONDS` em `.env.example`, junto dos TTLs e tetos já
  listados.

## 3. Tratamento de erro da API (`apps/api/src/app.ts`)

- [ ] 3.1 Passar `{ limit: config.requestBodyMaxBytes }` ao `express.json()` —
...
- [ ] 3.2 Reescrever o `errorHandler` para mapear `err.type === 'entity.too.large'`
  ...
  aparece na resposta.
- [ ] 3.3 Verificar com teste que o handler **não** propaga `err.status` de um
...
- [ ] 4.1 Recusar em `POST /files/upload-urls` a lista acima de
...
- [ ] 4.2 Verificar com teste que uma lista com exatamente o teto é aceita e
  processada normalmente (fronteira inclusiva).
- [ ] 4.3 Verificar que a recusa por lista vazia, já existente, continua
...
- [ ] 5.1 Fazer `getUploadUrl` do `gcs-storage-port.ts` assinar com
  ...
  continua válido.
- [ ] 5.2 Refletir o mesmo prazo no `in-memory-storage-port` usado pelos testes,
  para que a paridade dev↔prod do seam não se perca.
- [ ] 5.3 Verificar com teste que alterar o TTL de download não altera o
  `expiresAt` devolvido pelo envio.

## 6. Fila de transferência (`apps/web/src/upload`)

- [ ] 6.1 Extrair do `UploadArea.tsx` a orquestração de transferências para uma
...
- [ ] 8.1 Recusar no `startBatch`, antes da requisição, a seleção acima de
  `UPLOAD_BATCH_MAX_ITEMS_DEFAULT`, informando o teto — verificar com teste que
  nenhuma requisição é emitida.
- [ ] 8.2 Reconhecer `upload_batch_limit_exceeded` vindo do servidor em
...
- [ ] 9.1 Acrescentar a `docs/manual/docs/referencia/limites.md` a linha do teto
  de arquivos por requisição de envio, com a variável `UPLOAD_BATCH_MAX_ITEMS`,
  na tabela de limites vigentes.
- [ ] 9.2 Ajustar `docs/manual/docs/colaborador/enviar.md` para mencionar o teto
...
- [ ] 10.2 Verificar que `__tests__/web-serving.test.ts` continua passando sem
