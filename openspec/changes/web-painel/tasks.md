...
- [ ] 2.2 Criar `apps/web/src/painel/PainelPage.tsx`: quatro cartões
...
- [ ] 2.3 Estados: `Spin` no carregamento; `403` → `Result` de aviso neutro
      padrão; demais erros → `Result` genérico com tentar novamente; resposta
      zerada não é erro (design.md D5)
- [ ] 2.4 Trocar o `PlaceholderPage` pela `PainelPage` em
...
- [ ] 3.1 Admin vê os quatro cartões e os três blocos com os números do
      servidor (US 8.2 cenário 1), via `renderApp` + `mock-fetch`
- [ ] 3.2 Estado de carregamento exibido antes da resposta
- [ ] 3.3 Série de 12 meses: 12 barras, ordem cronológica, rótulos pt-BR,
      zeros visíveis
- [ ] 3.4 Categorias: sete rótulos pt-BR na ordem fixa, zero nas ausentes da
      resposta
- [ ] 3.5 Usado × disponível coerente com `storage` (proporção + absolutos,
      sem recálculo)
- [ ] 3.6 Repositório vazio: cartões e barras zerados, sem erro
- [ ] 3.7 `403` em `GET /dashboard` exibe o aviso neutro, sem números

## 4. Verificação e documentação

- [ ] 4.1 `npm run lint && npm run build && npm run test` verdes na raiz
- [ ] 4.2 Marcar a Fatia 10 como entregue em `docs/frontend_roadmap.md`
