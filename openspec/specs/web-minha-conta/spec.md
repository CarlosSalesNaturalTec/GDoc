# web-minha-conta Specification

## Purpose

Define os requisitos verificáveis da página "Minha conta" na SPA do GDoc — a
rota `/minha-conta`, acessível a qualquer pessoa autenticada, que exibe os
dados cadastrais somente para consulta (via `GET /auth/profile`) e oferece o
formulário de alteração da própria senha (via `POST /auth/password`),
garantindo que a senha nunca sobreviva ao formulário. Implementa o lado de
frontend da US 1.3 do PRD (`docs/prd_final.md`), consumindo as rotas da
capability `troca-de-senha` sem re-descrever seus cenários — o servidor
permanece o único guardião de permissão.
