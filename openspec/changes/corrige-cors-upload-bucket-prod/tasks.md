...
- [ ] 1.1 Obter o nome do bucket de arquivos:
  `terraform -chdir=infra/terraform output -raw files_bucket_name`.
- [ ] 1.2 Confirmar as origens de produção do SPA — as URL(s) do serviço Cloud Run
...
- [ ] 3.1 Em `infra/terraform/terraform.tfvars` (gitignored), definir
...
- [ ] 4.1 `terraform -chdir=infra/terraform plan` e revisar que a única mudança de
  CORS é a adição das origens (sem recriação do bucket).
- [ ] 4.2 `terraform -chdir=infra/terraform apply`.
