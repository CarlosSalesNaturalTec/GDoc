...
- [x] 1.1 `apps/web/src/lib/schemas.ts`: adicionar `personResponseSchema` tipado
...
- [x] 1.2 `apps/web/src/pessoas/queries.ts`: `useUsers()` como `useQuery`,
  `queryKey` `['users']`, chamando `GET /users` via `apiClient` e fazendo
  `.parse()` com `personListSchema` — design.md D7
- [x] 1.3 No mesmo arquivo: `useCreatePerson()` (`useMutation` → `POST /users`
...
- [x] 2.1 `apps/web/src/pessoas/PessoaFormModal.tsx`: `Modal` + `Form` que recebe
...
- [x] 2.2 Campos de perfil: nome (obrigatório), e-mail (obrigatório e **somente-
...
- [x] 2.3 Campo **senha**: presente e **obrigatório apenas no modo criar**;
...
- [x] 2.4 `Select` de **papel**: opções `collaborator`/`unit_admin` para todos e
  ...
  design.md D4
- [x] 2.5 Submit despacha para `useCreatePerson` (criar) ou `useUpdatePerson`
...
- [x] 2.6 Erro **409** no criar: exibir "e-mail já está em uso" associado ao campo
  de e-mail (`Form.setFields`/status de erro) **sem fechar** o modal nem limpar os
  demais campos — spec: e-mail duplicado; design.md D6
- [x] 2.7 Erro **403**: aviso neutro de permissão insuficiente via
...
- [x] 3.1 `apps/web/src/pessoas/PessoasPage.tsx`: consumir `useUsers()`; `Spin` em
...
- [x] 3.2 Botão **"Nova pessoa"** que abre o `PessoaFormModal` em modo criar;
  estado local do modal (pessoa em edição / criar) na página — design.md D1/D3
- [x] 3.3 Ações por-linha: **Editar** (abre o modal pré-preenchido) e
...
- [x] 3.4 Guarda de auto-tiro-no-pé: na linha do próprio admin (`row.id ===
...
- [x] 4.1 `apps/web/src/app/router.tsx`: trocar o `element` da rota
...
- [x] 5.1 `apps/web/src/__tests__/pessoas.test.tsx`: reusar `renderApp` +
...
- [x] 5.2 Cadastro válido: preencher e confirmar chama `POST /users`, fecha o
  modal e a nova pessoa aparece (invalidação); confirmar sem senha é bloqueado —
  spec: cadastro válido / senha é exigida
- [x] 5.3 E-mail duplicado: `POST /users` retornando 409 exibe "e-mail já está em
  uso" e mantém o modal aberto com os campos preenchidos — spec: e-mail duplicado
- [x] 5.4 Edição: alterar dados e confirmar chama `PATCH /users/:id` e reflete na
...
- [x] 5.5 Ativar/desativar: desativar chama `PATCH` com `status: 'disabled'` (após
...
- [x] 5.6 Travas de UX: `unit_admin` não vê `global_admin` no seletor de papel e
...
- [x] 6.2 `docs/frontend_roadmap.md`: marcar a **Fatia 9** como entregue (✅) e
