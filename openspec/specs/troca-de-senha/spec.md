# troca-de-senha Specification

## Purpose

Define os requisitos verificáveis da troca e redefinição de senha do GDoc —
a alteração da própria senha por qualquer pessoa autenticada, a política de
tamanho mínimo aplicada em todos os pontos de entrada, a redefinição
administrativa (com geração de senha e exibição única), o alcance por papel
da pessoa alvo, o encerramento das sessões abertas ao mudar a senha e a
consulta do próprio perfil. Implementa o Épico 1 / US 1.3 e US 1.4 do PRD
(`docs/prd_final.md`); os cenários Given/When/Then dessas US são vinculantes e
