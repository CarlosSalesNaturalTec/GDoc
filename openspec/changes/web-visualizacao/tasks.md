...
- [ ] 1.1 `apps/web/src/lib/schemas.ts`: adicionar `signedUrlResponseSchema` e
...
- [ ] 1.2 `apps/web/src/visualizacao/queries.ts`: hook `useViewUrl` como
  ...
  — design.md D3
- [ ] 1.3 Hook `useDownloadUrl` como `useMutation` que chama
  `POST /files/:id/download-url` e faz `.parse()` (`signedUrlResponseSchema`) —
  design.md D3

## 2. Fluxo de download

- [ ] 2.1 `apps/web/src/visualizacao/download.ts`: utilitário que, dada uma URL
...
- [ ] 3.1 `apps/web/src/visualizacao/PreviewModal.tsx`: `Modal` que, ao abrir,
...
- [ ] 3.2 Renderizadores por categoria (ramo `previewAvailable: true`): escolher
  ...
  design.md D2
- [ ] 3.3 Ramo `previewAvailable: false`: `Result`/`Empty` com mensagem
  **"pré-visualização indisponível"** + botão de download **apenas quando**
  ...
  design.md D5
- [ ] 3.4 Estados do modal: `Spin` durante a emissão da URL; tratamento de
...
- [ ] 4.1 `navegacao/ExplorerPage.tsx`: nome do arquivo vira clicável
  (`Button type="link"`) e nova ação **"Visualizar"** na coluna de ações, ambos
  abrindo o `PreviewModal` do arquivo da linha — design.md D1
- [ ] 4.2 Nova ação **"Baixar"** na coluna de ações do arquivo → `useDownloadUrl`
...
- [ ] 5.2 Visualizar arquivo de imagem renderiza inline (`<img>`) a partir da URL
  assinada; PDF renderiza em visualizador embutido (`<iframe>`) — US 9.2 cenário 1
- [ ] 5.3 Formato não suportado com `download.available: true` mostra mensagem de
  indisponibilidade + botão de download — US 9.2 cenário 2
- [ ] 5.4 Formato não suportado com `download.available: false` mostra a mensagem
  sem botão de download — US 9.2 cenário 2
- [ ] 5.5 Clicar em "Baixar" dispara `POST /files/:id/download-url` e a navegação
  para a URL assinada (âncora acionada) — RF #16
- [ ] 5.6 403 em view-url/download-url exibe aviso de permissão insuficiente, sem
  expor conteúdo do arquivo — RF #10

## 6. Documentação e verificação

- [ ] 6.1 `docs/frontend_roadmap.md`: marcar a Fatia 3 (`web-visualizacao`) como
  proposta/entregue
