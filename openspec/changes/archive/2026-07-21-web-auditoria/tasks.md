...
- [x] 1.1 `apps/web/src/lib/schemas.ts`: adicionar `auditQueryResponseSchema`
...
- [x] 1.2 `apps/web/src/auditoria/queries.ts`: hook `useFileAudit(fileId, open)`
...
- [x] 2.1 `apps/web/src/auditoria/AuditoriaModal.tsx`: `Modal` (título "Auditoria
...
- [x] 2.2 `Table` de eventos com colunas **Pessoa · Ação · Data/hora**: Pessoa =
  ...
  e-mail; design.md D4
- [x] 2.3 `Empty` "Nenhum acesso registrado" quando `events` é vazio (não é
  erro) — spec: arquivo sem acessos exibe estado vazio; design.md D5
- [x] 2.4 403 na consulta: `Result` de permissão insuficiente neutro via
...
- [x] 3.1 `apps/web/src/navegacao/ExplorerPage.tsx`: estado `auditingFile`
...
- [x] 3.2 Ação **"Auditoria"** só na linha de **arquivo** (nunca pasta), visível
...
- [x] 4.1 `apps/web/src/__tests__/auditoria.test.tsx`: reusar `renderApp` +
...
- [x] 5.2 `docs/frontend_roadmap.md`: marcar a **Fatia 8** como entregue (✅) e
